...
d3.csv('/data/Housing/House_Com.csv', function(StockData) {
  ...
  //console.log(nes_year) affichier objet 1 année inclue dans les données
...
//console.log(text);
  ...
  Final_data.forEach(function(d){
    SumData.push(d3.sum(keys, function(key) {return d[key]; }))
  });
  ...
  yBar.domain([0, d3.max(Final_data, function(d) {return d3.max(keys, function(key) {return d[key]; }); })]);
  ...
  var rect = group.selectAll("rect")
    ...
    .enter().append("rect")
      .attr("x", function(d) {return x(d.data.year); x(d.data.region);})
  ...
  function GroupeIt() {
    rect
      ...
      .transition().duration(1000)
        .attr("x", function(d) {return x(d.data.year) +  x1(this.parentNode.__data__.key)})
      .transition().duration(1000)
